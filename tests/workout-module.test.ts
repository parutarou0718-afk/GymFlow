...
test("WorkoutService creates, edits, restores, and completes a quick workout through GymFlowStore", async () => {
  ...
  const withSet = await service.addSet(
    started.id,
    withExercise.exercises[0].id,
  );
  const edited = await service.updateSet(
    started.id,
    withExercise.exercises[0].id,
    withSet.exercises[0].sets[0].setIndex,
    { weight: 60, reps: 8, completed: true },
  );

  assert.equal(edited.sourceType, "quick");
  assert.deepEqual(
    (await service.getWorkout(started.id))?.exercises[0].sets[0],
  ...
  assert.equal(
    (await service.getWorkoutHistoryDetail(started.id))?.id,
    started.id,
  );
  assert.equal(getCompletedExerciseCount(completed), 1);
  assert.equal(getCompletedVolume(completed), 480);
  assert.equal(
    (await store.events.getForSession(started.id)).filter(
...
test("Workout start accepts optional gymId without changing the default null behavior", async () => {
  ...
  assert.equal(
    (await scopedService.startQuickWorkout({ gymId: "gym-current" })).gymId,
    "gym-current",
  );
});
...
test("M18 replaces a zero-completed workout exercise in place with reset weight and provenance", async () => {
  ...
  assert.equal(
    (await store.events.getForSession(started.id)).filter(
      (event) => event.eventType === "WORKOUT_EXERCISE_REPLACED",
    ).length,
    1,
  );
});
...
test("M18 splits a partially completed workout exercise and preserves completed history", async () => {
  ...
  const started = await workout.startQuickWorkout();
  const original = await workout.addExercise(
    started.id,
    exerciseDB.getById("bench_press")!,
  );
  const entryId = original.exercises[0].id;
  for (let index = 0; index < 4; index += 1)
    await workout.addSet(started.id, entryId);
  await workout.updateSet(started.id, entryId, 0, {
    weight: 80,
    reps: 8,
    completed: true,
  });
  await workout.updateSet(started.id, entryId, 1, {
    weight: 80,
    reps: 8,
    completed: true,
  });
...
test("M21 Slice 5 persists completed facts for a partial replacement summary", async () => {
  const workout = createWorkoutService(createWebStore());
  const started = await workout.startQuickWorkout();
  const withExercise = await workout.addExercise(
    started.id,
    exerciseDB.getById("bench_press")!,
  );
  ...
  const replacementEntry = replaced.exercises[1];
  await workout.updateSet(started.id, replacementEntry.id, 0, {
    weight: 30,
    reps: 10,
    completed: true,
  });
  await workout.updateSet(started.id, replacementEntry.id, 1, {
    weight: 30,
    reps: 10,
    completed: true,
  });
  ...
  const persisted = await workout.getWorkoutHistoryDetail(started.id);

  assert.equal(completed.status, "completed");
  assert.equal(persisted?.status, "completed");
  assert.equal(
    persisted?.exercises.filter((item) =>
      item.sets.some((set) => set.completed),
    ).length,
    2,
  );
  assert.equal(
    persisted?.exercises.filter((item) => item.replacedFromExerciseId != null)
      .length,
    1,
  );
});

test("M21 Slice 5 keeps a completed no-Gym Quick Workout without a training location", async () => {
  const workout = createWorkoutService(createWebStore());
  const started = await workout.startQuickWorkout();
  const completed = await workout.finishWorkout(started.id);

  assert.equal(completed.gymId, null);
  assert.equal(
    (await workout.getWorkoutHistoryDetail(started.id))?.gymId,
    null,
  );
});
...
test("M18 rejects completed and stale entries without changing the Workout", async () => {
  const workout = createWorkoutService(createWebStore());
  const started = await workout.startQuickWorkout();
  const added = await workout.addExercise(
    started.id,
    exerciseDB.getById("bench_press")!,
  );
  ...
  await assert.rejects(
    () =>
      workout.replaceWorkoutExercise({
        ...
        replacementExerciseId: "dumbbell_bench_press",
        reason: "other",
        expectedCompletedSetCount: 1,
      }),
    /EXERCISE_ALREADY_COMPLETED/,
  );
  assert.deepEqual(await workout.getWorkout(started.id), beforeCompleted);

  const second = await workout.addExercise(
    started.id,
    exerciseDB.getById("incline_bench_press")!,
  );
...
for (const failureStage of ["session", "snapshot", "event"] as const) {
  test(`WorkoutService leaves no partial completion when atomic ${failureStage} persistence fails`, async () => {
    ...
    await assert.rejects(
      () => service.finishWorkout(started.id),
      /persistence failed/,
    );

    assert.equal((await service.getWorkout(started.id))?.status, "active");
    assert.equal(
      (await service.getWorkoutHistory()).some(
        (item) => item.id === started.id,
      ),
      false,
    );
    assert.equal(
      (await store.sync.getPending()).some(
        (item) => item.sessionId === started.id,
      ),
      false,
    );
    assert.equal(
      (await store.events.getForSession(started.id)).some(
        (event) => event.eventType === "WORKOUT_COMPLETED",
      ),
      false,
    );
    assert.notEqual(
      (await service.getWorkoutHistoryDetail(started.id))?.status,
      "completed",
    );
  });
}
