...
export default function CurrentGymScreen() {
  ...
  const adapt = async () => { if (!program) return; try { const next = await flow.createAdaptedProgramForCurrentGym({ userId: DEFAULT_LOCAL_USER_ID, programId: program.id, expectedGymId: currentGym?.id }); Alert.alert('Adapted Program created', next.name); } catch (reason) { showError(reason); } };
