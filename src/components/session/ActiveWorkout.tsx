...
import {
  createWorkoutReplacementService,
  type WorkoutReplacementOptions,
} from "../../modules/workout-replacement";
...
export function ActiveWorkout({
  ...
  const replacementApi = useMemo(
    () =>
      createWorkoutReplacementService({
        workouts: workoutApi,
        candidates: createCandidateResolutionService(store),
        matching: createMatchingService(store),
        exercises: createExerciseService(store),
      }),
    [store, workoutApi],
  );
  const [replacementOptions, setReplacementOptions] =
    useState<WorkoutReplacementOptions | null>(null);
  const [replacementExerciseId, setReplacementExerciseId] = useState<
    string | null
  >(null);
  const [replacementReason, setReplacementReason] =
    useState<WorkoutReplacementReason | null>(null);
  const [replacing, setReplacing] = useState(false);
  const [sessionGymName, setSessionGymName] = useState<string | null>(null);
  const [sessionGymUnavailable, setSessionGymUnavailable] = useState(false);
  useEffect(() => {
    const gymId = engine.session?.gymId;
    if (!gymId) {
      setSessionGymName(null);
      setSessionGymUnavailable(false);
      return;
    }
    void gymApi.getGym(gymId).then((gym) => {
      setSessionGymName(gym?.name ?? null);
      setSessionGymUnavailable(!gym);
    });
  ...
  const openReplacement = async (sessionExerciseId: string) => {
    ...
    try {
      ...
      Alert.alert(
        "Replace Exercise",
        error instanceof Error ? error.message : "Unable to load replacements",
      );
    }
  };
  const confirmReplacement = async () => {
    ...
    try {
      await replacementApi.replaceExercise({
        ...
        expectedCompletedSetCount: replacementOptions.completedSetCount,
      });
  ...
  const reasonLabels: Record<WorkoutReplacementReason, string> = {
    equipment_occupied: "Equipment is occupied",
    equipment_unavailable: "Equipment is unavailable",
    prefer_different_exercise: "I’d rather do another exercise",
    other: "Other",
  };
  ...
  const requestDiscardWorkout = () => {
    if (!engine.saving) setConfirmation("discard");
  };
      ...
      <View style={styles.confirmBackdrop}>
        <View style={styles.confirmCard}>
          <Text style={typography.h2}>
            {confirmation === "finish" ? "Finish Workout" : "Discard Workout"}
          </Text>
          <Text style={[typography.bodySmall, { marginTop: spacing.sm }]}>
            {confirmation === "finish"
              ? "Complete this workout?"
              : "Discard this workout? It will not appear in history."}
          </Text>
          <View style={styles.confirmActions}>
            ...
            >
              <Text
                style={{
                  color:
                    confirmation === "finish" ? colors.primary : colors.danger,
                  fontWeight: "700",
                }}
              >
                {confirmation === "finish" ? "Finish" : "Discard"}
              </Text>
            </TouchableOpacity>
          </View>
        </View>
  ...
  const replacementDialog = (
    <Modal
      transparent
      visible={replacementOptions !== null}
      animationType="slide"
      onRequestClose={() => setReplacementOptions(null)}
    >
      <View style={styles.confirmBackdrop}>
        <View style={styles.confirmCard}>
          <Text style={typography.h2}>Replace Exercise</Text>
          {replacementOptions?.gymValidation === "not_available" ? (
            <Text style={[typography.caption, { color: colors.textSecondary }]}>
              Suggestions are not verified against gym equipment.
            </Text>
          ) : null}
          ...
          {replacementOptions?.options.map((option, index) => (
            <TouchableOpacity
              disabled={replacing}
              key={option.exerciseId}
              onPress={() => setReplacementExerciseId(option.exerciseId)}
            ...
            >
              ...
              >
                {index === 0 ? "Recommended · " : ""}
                {option.name}
                {option.gymStatus === "executable_with_warning"
                  ? " · warning"
                  : ""}
              </Text>
            </TouchableOpacity>
          ))}
          {replacementOptions?.options.length === 0 ? (
            <Text style={{ color: colors.danger }}>
              No suitable replacement is available at this Gym.
            </Text>
          ) : null}
          <Text style={[typography.caption, { marginTop: spacing.md }]}>
          ...
          <View style={styles.confirmActions}>
            <TouchableOpacity
              disabled={replacing}
              onPress={() => setReplacementOptions(null)}
            >
              <Text style={{ color: colors.textSecondary, fontWeight: "700" }}>
                Cancel
              </Text>
            ...
            <TouchableOpacity
              disabled={
                replacing || !replacementExerciseId || !replacementReason
              }
              onPress={() => void confirmReplacement()}
            >
  ...
  if (pickingExercise) {
    return (
      <ExercisePicker
        selectedIds={engine.session.exercises.map(
          (exercise) => exercise.exerciseId,
        )}
        onSelect={async (exercise) => {
          await engine.addExercise(exercise);
        ...
        <View style={styles.header}>
          <View>
            ...
            <Text style={typography.caption}>
              {engine.session.gymId
                ? sessionGymName
                  ? `Training at ${sessionGymName}`
                  : sessionGymUnavailable
                    ? "Training location unavailable"
                    : "Training location unavailable"
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ExerciseBlock
              ...
              onRemoveExercise={(id) => {
                const exercise = engine.session?.exercises.find(
                  (item) => item.id === id,
                );
                if (
                  !exercise?.sets.some(
                  ...
                  void engine.removeExercise(id);
                  return;
                }
                Alert.alert(
                  "Remove Exercise",
                  "This exercise has recorded sets. Remove it?",
                  [
                    { text: "Cancel", style: "cancel" },
                    {
                      text: "Remove",
                      style: "destructive",
                      onPress: () => void engine.removeExercise(id),
                    },
                  ],
                );
              }}
          ...
          ListEmptyComponent={
            <View style={styles.empty}>
              <Text style={typography.bodySmall}>
                No exercises yet. Add one to start your quick workout.
              </Text>
            </View>
          }
          ListFooterComponent={
